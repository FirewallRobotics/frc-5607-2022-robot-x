...
public void execute() {
    ...
    if (x_Blue == -1) {
        ...
        m_vision.ScaledRadius =  m_vision.PGainR * ((((m_vision.MaxOutR - m_vision.MinOutR)*(blue_radius - m_vision.MinRadius)/(MaxRadius - m_vision.MinRadius))) + m_vision.MinOutR) -  m_vision.NeutralOffSetR;
    }
    if (x_Blue == center) {
        m_drivetrain.Drive(0.25, 0, 0); }    
    else if (x_Blue < center) {
        m_drivetrain.Drive(0, -0.25, 9); }
    else if (x_Blue > center) {
        m_drivetrain.Drive(0, 0.25, 9);
    }
...
//         return (m_drivetrain.ScaledRadius >= m_drivetrain.MaxRadius);
